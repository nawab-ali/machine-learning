 ...
 */
template <class T> vector<Point<double>> get_random_points(unsigned int k, const vector<Point<T>> &points) {
 ...
 * @return Void
 */
template <class T> void init_cluster_centroid(vector<Cluster<T>> &clusters, vector<Point<double>> &random_points) {
    int i = 0;
    ...
    for_each(clusters.begin(), clusters.end(),
             [&random_points, &i](Cluster<T> &cluster) { cluster.set_centroid(random_points[i++]); });
}
 ...
 */
template <class T> double euclidean_distance(const Point<T> &point1, const Point<T> &point2) {
    double distance = 0.0;
 ...
 */
template <class T> int get_nearest_cluster(const Point<T> &point, vector<Cluster<T>> &clusters) {
    if (clusters.size() == 0) {
 ...
 * @return None
 */
template <class T> void categorize_points_into_clusters(vector<Point<T>> &points, vector<Cluster<T>> &clusters) {
    ...
    for_each(clusters.begin(), clusters.end(), [](Cluster<T> &cluster) { cluster.update_centroid(); });
}
 ...
 * @return None
 */
template <class T> void remove_points_from_clusters(vector<Cluster<T>> &clusters) {
    vector<Point<T>> empty_points;

    for_each(clusters.begin(), clusters.end(),
             [empty_points](Cluster<T> &cluster) { cluster.set_points(empty_points); });
}
 ...
 */
template <class T> vector<Cluster<T>> KMeans(unsigned int k, vector<Point<T>> &points, long max_iterations = 1000) {
