...
template <class T> class Point {
  public:
    ...
    friend ostream &operator<<(ostream &out, const Point<T> &point) {
        for_each(point.coordinates.begin(), point.coordinates.end(), [&out](const T e) { out << e << " "; });
...
template <class T> Point<T>::Point(int dimensions) { coordinates = vector<T>(dimensions); }

template <class T> Point<T>::Point(vector<T> coordinates) : coordinates(coordinates) {}

// Copy Constructor
template <class T> Point<T>::Point(const Point<T> &point) : Point(point.coordinates) {}
...
// Overload == operator
template <class T> bool Point<T>::operator==(const Point<T> &point) const { return (coordinates == point.coordinates); }

template <class T> int Point<T>::get_dimensions(void) const { return (coordinates.size()); }

template <class T> vector<T> Point<T>::get_coordinates(void) const { return (coordinates); }

template <class T> void Point<T>::set_coordinates(const vector<T> coordinates) { this->coordinates = coordinates; }
