...
template <class T> class Cluster {
  public:
    ...
    friend ostream &operator<<(ostream &out, const Cluster<T> &cluster) {
        ...
        for_each(cluster.points.begin(), cluster.points.end(), [&out](const Point<T> p) { out << p << " " << endl; });
...
template <class T> Cluster<T>::Cluster(int id, vector<Point<T>> points) : id(id), points(points) {}

// Copy Constructor
template <class T> Cluster<T>::Cluster(const Cluster<T> &cluster) : Cluster(cluster.id, cluster.points) {
    centroid = cluster.centroid;
}

// Overload assignment operator
template <class T> Cluster<T> &Cluster<T>::operator=(const Cluster<T> &cluster) {
    id = cluster.id;
    centroid = cluster.centroid;
...
// Overload == operator
template <class T> bool Cluster<T>::operator==(const Cluster<T> &cluster) const {
    return (id == cluster.id && points == cluster.points);
}

template <class T> int Cluster<T>::get_id(void) const { return (id); }

template <class T> Point<double> Cluster<T>::get_centroid(void) const { return (centroid); }

template <class T> vector<Point<T>> Cluster<T>::get_points(void) const { return (points); }

template <class T> void Cluster<T>::set_id(const int id) { this->id = id; }

template <class T> void Cluster<T>::set_centroid(const Point<double> centroid) { this->centroid = centroid; }

template <class T> void Cluster<T>::set_points(const vector<Point<T>> points) { this->points = points; }

template <class T> void Cluster<T>::add_point(const Point<T> point) { points.push_back(point); }
...
template <class T> long Cluster<T>::get_num_points(void) const { return (points.size()); }
...
template <class T> void Cluster<T>::update_centroid(void) {
    ...
    for (int i = 0; i < coordinates[0].size(); i++) {
        ...
        col_vector_avg = accumulate(col_vector.begin(), col_vector.end(), 0.0) / static_cast<double>(col_vector.size());
...
// Create 2D vector of the point coordinates in cluster
template <class T> vector<vector<T>> Cluster<T>::create_vec2d_coordinates(void) {
    vector<vector<T>> coordinates;

    for_each(points.begin(), points.end(),
             [&coordinates](const Point<T> point) { coordinates.push_back(point.get_coordinates()); });
