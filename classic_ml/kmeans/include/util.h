 ...
 */
template <class T> vector<T> tokenize(const string &str, char delimiter, unsigned int num_features) {
    string token;
    vector<T> tokens;
    int token_count = 0;
 ...
 */
template <class T> vector<vector<T>> read_training_data(const string &training_data_file, unsigned int num_features) {
    string line;
    ifstream file;
 ...
 */
template <class T> vector<T> get_column_vector(const vector<vector<T>> &vec2d, unsigned int index) {
    vector<T> column;
